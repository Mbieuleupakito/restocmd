...
function AutoScrollGrid({ commandes, now, alarmCmds, onStatut }: {
  ...
  return (
    <div ref={containerRef}
      ...
      onMouseEnter={() => { if (scrollRef.current) { clearInterval(scrollRef.current); scrollRef.current = null } }}
      onTouchStart={() => { if (scrollRef.current) { clearInterval(scrollRef.current); scrollRef.current = null } }}
    >
      {commandes.map(cmd => (
