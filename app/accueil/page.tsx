...
const SOURCE_STYLES: Record<Source, string> = {
  'Présentiel': 'source-presentiel',
  'À emporter': 'source-emporter',
  'Deliveroo': 'source-deliveroo',
  'Uber Eats': 'source-ubereats',
}
...
export default function AccueilPage() {
  ...
  const loadCommandes = useCallback(async () => {
    ...
    if (data) {
      setCommandes(data.map((c: any) => ({ ...c, lignes: c.lignes_commande || [] })))
    }
  }, [])

  useEffect(() => {
    loadCommandes()
    const channel = supabase.channel('accueil')
  ...
  const ajouterAuPanier = () => {
    ...
    const ligne: LigneCommande = {
      plat_id: platActif.id,
      nom: platActif.nom,
      complement_id: complementId,
      complement_nom: comp?.nom || null,
      quantite,
  ...
  const envoyerCommande = async () => {
    ...
    try {
      const { data: cmd, error } = await supabase.from('commandes').insert({
        source, table_ref: tableRef, statut: 'nouvelle',
        montant_total: totalPanier, notes,
      }).select().single()
      if (error || !cmd) throw error

      const lignesInsert = panier.map(l => ({
  ...
  const stats = {
    ...
    total: commandes.filter(c => c.statut !== 'servie').length,
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
      ...
      <div className="stat-bar">
        ...
        <div className="stat-item" style={{ marginLeft:'auto' }}>
          <button className="btn-primary" onClick={() => setShowForm(true)} style={{ padding:'8px 16px', fontSize:'0.85rem' }}>
            + Commande
          </button>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px' }}>
        {commandes.length === 0 ? (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {commandes.map(cmd => (
              <CommandeCard key={cmd.id} cmd={cmd} onStatut={changerStatut} onFacture={() => setShowFacture(cmd)} />
            ))}
          </div>
        )}
      </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal" style={{ maxWidth:'600px' }}>
            ...
            <div className="modal-body">
              ...
              <input value={tableRef} onChange={e => setTableRef(e.target.value)} placeholder="Table 3 / Commande #1234" style={{ marginBottom:'16px' }} />
              ...
              <select value={platSelId||''} onChange={e => { setPlatSelId(Number(e.target.value)); setComplementId(null); const p = PLATS.find(x=>x.id===Number(e.target.value)); setPrixChoisi(p?.prix||0) }} style={{ marginBottom:'10px' }}>
                <option value="">— Sélectionner un plat —</option>
                {platsCategorie.map(p => (
                  <option key={p.id} value={p.id}>{p.nom} — {p.prix}€{p.prix2?` / ${p.prix2}€`:''}</option>
                ))}
              </select>

              {platActif && platActif.prix2 && (
                <div style={{ marginBottom:'10px' }}>
                  <label className="field-label">Taille / Portion</label>
              ...
              {platActif?.complement && (
                <div style={{ marginBottom:'10px' }}>
                  <label className="field-label">Complément (inclus)</label>
                  <select value={complementId||''} onChange={e => setComplementId(Number(e.target.value)||null)}>
                    <option value="">Sans complément</option>
                    {COMPLEMENTS.map(c => (
                      <option key={c.id} value={c.id}>{c.nom}{c.prix_supplement>0?` (+${c.prix_supplement}€)`:' (inclus)'}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              {panier.length > 0 && (
                <div style={{ background:'var(--surface2)', borderRadius:'var(--radius)', padding:'14px', marginBottom:'16px' }}>
                  <div style={{ fontFamily:'var(--font-display)', fontSize:'1rem', letterSpacing:'2px', marginBottom:'10px', display:'flex', justifyContent:'space-between' }}>
                    <span>PANIER</span>
                    <span style={{ color:'var(--gold)' }}>{totalPanier}€</span>
                  </div>
                  {panier.map((l, i) => (
              ...
              <label className="field-label">Notes cuisine (optionnel)</label>
  ...
  return (
    <div className={`commande-card ${cmd.statut}`}>
      <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', cursor:'pointer' }} onClick={() => setOpen(o=>!o)}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          <span style={{ fontFamily:'var(--font-display)', fontSize:'1.4rem', color: cmd.statut==='nouvelle'?'var(--red)':cmd.statut==='en_preparation'?'var(--yellow)':cmd.statut==='prete'?'var(--green)':'var(--text2)' }}>
        ...
        <div style={{ borderTop:'1px solid var(--border)', padding:'12px 16px' }}>
          {plats.length > 0 && (
            <div style={{ marginBottom:'8px' }}>
              <div style={{ fontSize:'0.65rem', color:'var(--red)', letterSpacing:'2px', marginBottom:'6px' }}>🍛 PLATS (cuisine)</div>
              {plats.map((l,i) => (
                <div key={i} style={{ fontSize:'0.85rem', padding:'3px 0', color:'var(--text)' }}>
                  {l.quantite}× {l.nom}{l.complement_nom?` + ${l.complement_nom}`:''}{l.remarque?` · ${l.remarque}`:''}
                  <span style={{ color:'var(--gold)', marginLeft:'8px' }}>{l.sous_total}€</span>
                </div>
              ))}
            </div>
          )}
          {boissons.length > 0 && (
            <div style={{ marginBottom:'8px' }}>
              <div style={{ fontSize:'0.65rem', color:'var(--blue)', letterSpacing:'2px', marginBottom:'6px' }}>🥤 BOISSONS (accueil)</div>
              {boissons.map((l,i) => (
                <div key={i} style={{ fontSize:'0.85rem', padding:'3px 0' }}>
                  {l.quantite}× {l.nom} <span style={{ color:'var(--gold)', marginLeft:'8px' }}>{l.sous_total}€</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display:'flex', gap:'8px', marginTop:'12px', flexWrap:'wrap' }}>
            ...
            {cmd.statut === 'en_preparation' && <button className="btn-green" onClick={() => onStatut(cmd.id,'prete')} style={{ fontSize:'0.78rem', padding:'7px 12px' }}>✅ Marquer prête</button>}
...
function StatutBadge({ statut }: { statut: Statut }) {
  const map = { nouvelle:{label:'Nouvelle',cls:'badge-red'}, en_preparation:{label:'En prép.',cls:'badge-gray'}, prete:{label:'Prête ✓',cls:'badge-green'}, servie:{label:'Servie',cls:'badge-gray'} }
  const s = map[statut]
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target===e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth:'480px' }}>
        ...
        <div className="modal-body" id="facture-print">
          {/* EN-TÊTE */}
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'16px', fontSize:'0.82rem' }}>
            <div><div style={{ color:'var(--text2)' }}>N° Commande / Order #</div><div style={{ fontFamily:'var(--font-display)', fontSize:'1.2rem' }}>#{String(cmd.numero||cmd.id).padStart(4,'0')}</div></div>
            <div style={{ textAlign:'right' }}><div style={{ color:'var(--text2)' }}>Date / Date</div><div>{date}</div><div style={{ color:'var(--text3)', fontSize:'0.75rem' }}>{heure}</div></div>
          </div>

          {cmd.table_ref && <div style={{ fontSize:'0.82rem', marginBottom:'12px' }}><span style={{ color:'var(--text2)' }}>Table / Réf : </span>{cmd.table_ref}</div>}
          ...
          <div style={{ borderTop:'1px solid var(--border)', paddingTop:'12px', marginBottom:'12px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.7rem', color:'var(--text3)', marginBottom:'8px', letterSpacing:'1.5px', textTransform:'uppercase' }}>
              <span>Article / Item</span><span>Qté</span><span>Prix / Price</span><span>Total</span>
            </div>
            {(cmd.lignes||[]).map((l,i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:'0.85rem', padding:'5px 0', borderBottom:'1px solid var(--border)' }}>
                <div style={{ flex:3 }}>
                  {l.nom}
                  {l.complement_nom && <div style={{ fontSize:'0.72rem', color:'var(--text3)' }}>+ {l.complement_nom}</div>}
                  {l.remarque && <div style={{ fontSize:'0.72rem', color:'var(--text3)', fontStyle:'italic' }}>{l.remarque}</div>}
                </div>
          ...
          <div style={{ background:'var(--surface2)', borderRadius:'var(--radius)', padding:'12px 16px', marginBottom:'16px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.82rem', color:'var(--text2)', marginBottom:'6px' }}>
              <span>Sous-total / Subtotal</span><span>{cmd.montant_total}€</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.82rem', color:'var(--text2)', marginBottom:'10px' }}>
              <span>TVA / VAT (incluse)</span><span>—</span>
            </div>
        ...
        <div style={{ padding:'0 20px 20px', display:'flex', gap:'10px' }}>
          ...
          <button className="btn-primary" onClick={print} style={{ flex:2 }}>🖨️ Imprimer / Print</button>
        </div>
      </div>
    </div>
  )
